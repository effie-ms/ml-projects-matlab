function [predictions,b] = PredictMyLinearRegression(trainSet,testSet,coeffMethod,learningRate, n)
...
function [b,b0] = GetCoefficientsBatchGradientDescent(X, y, learningRate, nBatch)
    ...
    for batch = 1:nBatch
        ...
        error = yhat - y;
        coef = coef - (learningRate/nX)*X'*error;
    end
    b0 = coef(1);
    b = coef(2:end);
end
...
function [b, b0] = GetCoefficientsStochasticGradientDescent(X,y, learningRate, nEpoch)
    ...
    for epoch = 1:nEpoch % number of passes
        for j = 1:nX
			...
			error = yhat - y(j);
            coef = coef - learningRate * error .* X(j,:);
        end
    end
    b0 = coef(1);
    b = coef(2:end);
